...
| Agent Alias | TSTALIASID | DRAFT alias — always uses latest agent state |
| Agent Alias | BM6JROSWME ("prod") | Points to version 1 (Feb 27 baseline) |
| LangFuse | cloud.langfuse.com | LLM tracing + observability (ACTIVE as of Mar 1 2026) |
...
5. **TSTALIASID (DRAFT)**: Always uses latest prompt/action groups. The `prod` alias (BM6JROSWME) points to frozen v1 (Feb 27). DRAFT is intentionally used to reflect all prompt changes without a separate publish step. Suitable for rapid iteration during hackathon.
...
14. **LangFuse tracing**: `mandimitra-chat` Lambda sends trace data to LangFuse (cloud.langfuse.com) for every agent invocation. Each Bedrock Agent step (reasoning, tool call, observation, model output) is logged as a span under a session trace. Enables latency monitoring, error tracking, and conversation analysis.

15. **WhatsApp sharing (not PDF)**: All bot responses include a WhatsApp share button (`wa.me/?text=...`). No PDF generation or file download is provided — sharing is text-based via WhatsApp. This is intentional: WhatsApp works on all phones including feature phones with slow connections.
