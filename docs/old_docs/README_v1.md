...
MandiMitra uses **Amazon Bedrock Agents** with **4 action groups** and **13 tool functions** to provide comprehensive market intelligence:

| Capability | What It Does |
|-------|------|
| **Price Intelligence** | Real-time mandi prices, nearby mandis (GPS-based, 50km radius), trend analysis |
| **Smart Sell Advisory** | AI-powered SELL / HOLD / SPLIT with shelf life, storage tips, weather risk, price prediction |
| **Negotiation Prep** | Generates shareable price briefs for mandi negotiation |
| **Weather Advisory** | 5-day agricultural weather forecast and sell-timing guidance |
| **Browse & Mandi Tools** | Dynamic commodity lists, mandi profiles, state-wise data browsing |

**20 commodities** tracked across **14 states** with **60+ mandi GPS coordinates**. All through a **conversational Hindi interface** where farmers ask questions naturally and receive data-driven, actionable answers.
...
User (Hindi/English/Voice)
   ...
   [CloudFront HTTPS CDN]
        │
   [S3 Static Website + PWA]
        │
   [API Gateway]
        │
  [Orchestrator Agent] ── Amazon Bedrock (Nova Pro)
        │
  [4 Action Groups, 13 Functions]
  ├── PriceIntelligenceTools (5 functions)
  ├── MandiTools (2 functions)
  ├── BrowseTools (3 functions)
  └── WeatherTools (1 function)
        │
  [Tool Layer]
  ├── DynamoDB (live prices, 20 commodities x 14 states)
  ├── Agmarknet API (data.gov.in, daily 9:30 PM IST)
  ├── Open-Meteo Weather API (5-day forecast)
  ├── GPS Distance Calculator (Haversine)
  └── MSP + Shelf Life + Storage Lookup
```
...
### 5. Load Data
```bash
# Trigger data ingestion manually (or wait for daily 9:30 PM IST schedule)
...
2. **Best Mandi (GPS):** *"मेरे पास 20 क्विंटल गेहूं है, कहाँ बेचूं?"* (finds mandis within 50km)
3. **Smart Sell Advisory:** *"क्या अभी बेचना चाहिए या रुकूं?"* (includes shelf life, storage cost, weather risk, price prediction)
4. **Weather:** *"मौसम कैसा है अगले 5 दिन?"*
5. **Negotiation Brief:** *"Price brief दो गेहूं का"*
6. **Browse:** *"कौन सी फसलें हैं?"* (dynamic, state-specific crop lists)
